...
import { DAGLayoutEngine } from '../../src/webview/components/GraphCanvas/dag-layout';
...
describe('DAGLayoutEngine', () => {
  ...
  it('lays out a linear commit chain with non-overlapping nodes', () => {
    ...
    // In top-to-bottom layout, y coordinates should be strictly ordered
    ...
    // Node 3 (newest) should have a distinct y from Node 2 and Node 1
    expect(node3.y).not.toBe(node2.y);
    expect(node2.y).not.toBe(node1.y);

    // Edges should have spline points
    expect(layout.edges[0]?.points.length).toBeGreaterThanOrEqual(2);
  });

  it('lays out a 2-parent merge commit and flags isMerge: true', () => {
    const commits: CommitNode[] = [
    ...
    // Merge node should have edges to both parents
    const mergeEdges = layout.edges.filter((e) => e.source === 'c4');
    expect(mergeEdges.length).toBe(2);
  });

  it('lays out an octopus merge with 3 parents', () => {
    const commits: CommitNode[] = [
  ...
  it('supports Left-to-Right (LR) orientation', () => {
    const commits: CommitNode[] = [
      { hash: 'c2', shortHash: 'c2', subject: 'second', author: 'A', authorEmail: 'a@a.com', date: new Date(), parents: ['c1'], refs: ['HEAD -> main'] },
      { hash: 'c1', shortHash: 'c1', subject: 'first', author: 'A', authorEmail: 'a@a.com', date: new Date(), parents: [], refs: [] },
    ];

    const layoutLR = engine.layout(commits, [], new Map(), { direction: 'LR' });
    const node1 = layoutLR.nodeMap.get('c1')!;
    const node2 = layoutLR.nodeMap.get('c2')!;

    // In LR layout, X coordinates differ along time rank
